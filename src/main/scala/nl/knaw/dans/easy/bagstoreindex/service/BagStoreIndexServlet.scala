...
case class BagStoreIndexServlet(app: BagStoreIndexApp) extends ScalatraServlet with DebugEnhancedLogging {
  import app._
  val externalBaseUri = new URI(properties.getString("bag-store-index.daemon.external-base-uri"))

  get("/") {
    Ok("Hello world")
  }

  // get: http://bag-store-index/bag-sequence?contains=<bagId>
  ...
  // put: http://bag-store-index/bags/<bagId>
  ...
  // get: http://bag-store-index/bags/<bagId>
